...
public class SpatialPartition : MonoBehaviour
{
        ...
        for (int x = 0; x < m_rows; ++x)
        {
            for (int y = 0; y < m_columns; ++y)
            {
                ...
                //for (int i = 0; i < SPGridMesh[x, y].ObjectList.Count; ++i)
                {
                    ...
                    if (obj.transform.position.x < (x * GridSizeX) - GridSizeX * 0.5f ||
                        obj.transform.position.x > (x * GridSizeX) + GridSizeX * 0.5f ||
                        obj.transform.position.z < (y * GridSizeZ) - GridSizeZ * 0.5f ||
                        obj.transform.position.z > (y * GridSizeZ) + GridSizeZ * 0.5f)
                    {
                        toBeRemoved.Add(obj);
    ...
    void MigrationUpdate()
    {
        while (MigrationList.Count > 0)
        {
            ...
            int index_Y = (int)(MigrationList[0].transform.position.y / GridSizeZ);
